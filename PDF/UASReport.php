...
class PDF extends TCPDF {
    ...
    function Header() {
        ...
        //Judul dalam bingkai
        $this->Cell(0, 5, 'Laporan Hasil Penilaian Semester Genap', 0, 1, 'C');
...
$pdf->SetTitle('Laporan Hasil Penilaian Semester Genap '.$periode->getPeriodename());
...
$pdf->Cell(0, 0, 'Laporan Hasil Penilaian Semester Genap '.$periode->getPeriodename(), 1, 1, 'C');
$pdf->Cell(0, 0, '', 0, 1, 'C');
...
while ($iterator -> valid()) 
{
    ...
    $tbl .= '<td>'.$kkm.'</td>';
    //nilai angka
    $nilai = cariNilaiAkhirUAS($iterator->current());
    //$nilai = 100;
    if($nilai < $kkm)
    {
...
while($students->valid())
{
    ...
    while($iterator->valid())
    {
        $totalnilai = cariNilaiAkhirUAS($iterator->current());
        $tmp += $totalnilai;
        $iterator->next();
    }
    if($tmp > $jumlah)
